        ...
        module.directive('inspireTree', function() {
            return {
                ...
                link: function(scope) {
                    ...
                    // Instatiate InspireTree with given options
                    var tree = new InspireTree(scope.options);
                    scope.tree = tree;

                    // When the tree has loaded the nodes, assign to our scope
                    // so the template can properly render
                    tree.on('model.loaded', function(model) {
                        scope.model = model;
                    });

                    // If the user asks for the tree api, give it
                    if (typeof scope.options.onRegisterApi === 'function') {
                        scope.options.onRegisterApi(tree);
                    }

                    // Watch for changes to the data nodes/function so we can reload the tree
